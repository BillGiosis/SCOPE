"""Main training script for CLAD-C benchmark."""

import argparse
import os
import sys

os.environ['CLAD_SUPPRESS_DETECTRON_MSG'] = '1'

import torch

sys.path.append(os.path.dirname(os.path.dirname(__file__)))

from benchmarks.cladc import create_cladc_benchmark
...
from utils.reports_cladc import write_cladc_results
...
def main(args):
    ...
    print("\nLoading CLAD-C benchmark")
    benchmark = create_cladc_benchmark(config)
    ...
    results_file = write_cladc_results(config, trainer, timestamp)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Train on CLAD-C benchmark')
    parser.add_argument('--config', type=str, default='config/cladc_config.yaml', help='Path to configuration file')
    parser.add_argument(
        '--device',
        type=str,
